...
namespace UnityPMSManager
{
    public class GoogleSheetExporter
    {
        ...
        {
            var result = false;
            StreamWriter tableSw = new StreamWriter(data.tablePath + "/" + "CustomTableName.txt");
            StreamWriter exportSw = new StreamWriter(data.exportPath + "/" + "CustomExportTableName.txt");
            ...
            LoadGoogleSheet(url, (resBool, json) =>
                {
                    if (resBool)
                    {
                        ...
                        {
                            tableSw.Write(AES.Encode(json, data.crypto));
                        }
                        else
                        {
                            tableSw.Write(json);
